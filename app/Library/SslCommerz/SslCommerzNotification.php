...
//                    }
            ...
            # ADD hashed OF STORE PASSWORD
            $new_data['store_passwd'] = Hash::make($store_passwd);
            ...
            if (Hash::make($hash_string) == $post_data['verify_sign']) {
	...
	protected function setSuccessUrl() {
		...
		$this->successUrl = \Config::get('app.url') . $this->config['success_url'];
    }
	...
	protected function setFailedUrl() {
		$this->failedUrl = \Config::get('app.url') . $this->config['failed_url'];
    }
	...
	protected function setCancelUrl() {
		$this->cancelUrl =\Config::get('app.url') . $this->config['cancel_url'];
    }
	...
	protected function setIPNUrl() {
		$this->ipnUrl =\Config::get('app.url') . $this->config['ipn_url'];
	}
