...
<div class="container-fluid  dashboard-content">
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h2 class="pageheader-title">{{ __('hub_payment.edit_payment') }}</h2>
                    <form action="{{route('hub.hub-payment.update',$singlePayment->id)}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        @method('PUT')
                        @csrf
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select  id="hub_id" class="form-control d" name="hub_id">
                                        ...
                                        @foreach ($hubs as $hub)
                                            <option value="{{ $hub->id }}"  @if ($singlePayment->hub_id == $hub->id) selected @endif>{{ $hub->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('hub_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="amount" type="number" name="amount" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_Amount') }}" autocomplete="off" class="form-control  @error('amount') is-invalid @enderror" value="{{old('amount',$singlePayment->amount)}}" require>
                                    @error('amount')
                                    <small class="text-danger mt-2">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="process" @if ($errors->has('transaction_id') || $errors->has('from_account') || $singlePayment->status == \App\Enums\ApprovalStatus::PROCESSED) @else style="display: none" @endif>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input id="transaction_id" type="text" name="transaction_id" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_Transaction_ID') }}" autocomplete="off" class="form-control @error('transaction_id') is-invalid @enderror" value="{{old('transaction_id',$singlePayment->transaction_id)}}" require>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('from_account') is-invalid @enderror" name="from_account" >
                                            <option disabled selected>{{ __('menus.select') }} {{ __('hub_payment.from_account') }}</option>
                                            ...
                                            @if ($account->gateway == 1)
                                                ...
                                                @elseif($account->gateway == 3 || $account->gateway == 4 || $account->gateway == 5)
                                                    <option value="{{ $account->id }}" @if($account->id == $singlePayment->from_account) selected @endif>{{$account->account_holder_name}} |No : {{ $account->mobile }}|  @if($account->type == 1) Merchant @else Persional @endif | {{ __('hub_payment.current_balance') }}: {{ $account->balance }} </option>
                                                @else
                                                    <option value="{{ $account->id }}" @if($account->id == $singlePayment->from_account) selected @endif>{{$account->account_holder_name}} | A.No : {{ $account->account_no }} | {{ __('hub_payment.current_balance') }}: {{ $account->balance }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                        @error('from_account')
