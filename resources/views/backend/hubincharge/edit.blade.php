    ...
    <div class="container-fluid  dashboard-content">
        ...
        <div class="row">
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="card">
                    <div class="card-body">
                        <h2 class="pageheader-title"> {{ __('incharge.edit') }} {{ $hub->name }}
                            {{ __('incharge.title') }}</h2>
                        <form action="{{ route('hub-incharge.update', [$hub->id, $inCharge]) }}" method="POST"
                                                ...
                                                @foreach ($users as $key => $user)
                                                    @if ($user->id != 1)
                                                        <option value="{{ $user->id }}"
                                                            {{ old('user_id', $inCharge->user_id) == $user->id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="status" class="form-control @error('status') is-invalid @enderror">
                                            @foreach (trans('status') as $key => $status)
                                                <option value="{{ $key }}"
                                                    {{ old('status', $inCharge->status) == $key ? 'selected' : '' }}>
                                                    {{ $status }}</option>
                                            @endforeach
                                        </select>
                                        @error('status')
