...
@section('title')
   {{ __('levels.partner') }} {{ __('levels.edit') }}
@endsection
@section('maincontent')
                    ...
                    <form action="{{route('partner.update',$partner->id)}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input id="name" type="text" name="name" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_name') }}" autocomplete="off" class="form-control @error('name') is-invalid @enderror" value="{{old('name',@$partner->name)}}" require>
                                @error('name')
                            ...
                            <div class="form-group  col-md-6">
                                ...
                                <input id="link" type="text" name="link" data-parsley-trigger="change" placeholder="{{ __('levels.enter_link') }}" autocomplete="off" class="form-control @error('link') is-invalid @enderror" value="{{old('link',@$partner->link)}}" >
                                @error('link')
                                    <small class="text-danger mt-2">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="form-group    col-md-6">
                                ...
                                <input id="position" type="text" name="position" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_Position') }}" autocomplete="off" class="form-control @error('position') is-invalid @enderror" value="{{old('position',@$partner->position)}}" >
                                @error('position')
                                    <small class="text-danger mt-2">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="form-group  col-md-6">
                                ...
                                <select name="status" class="form-control @error('status') is-invalid @enderror">
                                    @foreach(trans('status') as $key => $status)
                                        <option value="{{ $key }}" {{ (old('status',$partner->status) == $key) ? 'selected' : '' }}>{{ $status }}</option>
                                    @endforeach
                                </select>
                                @error('status')
                                    <small class="text-danger mt-2">{{ $message }}</small>
