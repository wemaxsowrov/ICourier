...
@section('title')
    {{ __('permissions.cash_received_from_delivery_man') }} {{ __('levels.edit') }}
@endsection
@section('maincontent')
<div class="container-fluid  dashboard-content">
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h2 class="pageheader-title">{{ __('levels.edit') }}</h2>
                    <form action="{{route('cash.received.deliveryman.update',['id'=>$singleCashAmount->id])}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        @csrf
                        @method('put')
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group delivery_man_search">
                                    ...
                                    <div  class="deliveryman_balance active show">Current balance : {{$singleCashAmount->deliveryman->current_balance}}</div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="date" data-toggle="datepicker" name="date" data-parsley-trigger="change" placeholder="yyyy-mm-dd"  class="form-control" value="{{old('date', \Carbon\Carbon::parse($singleCashAmount->date)->format('Y-m-d'))}}" required>
                                    @error('date')
                                    <small class="text-danger mt-2">{{ $message }}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <select id="account_ids" name="account_id" class="form-control" required>
                                        ...
                                        @foreach($accounts as $account)
                                            <option {{ (old('account_id',$singleCashAmount->account_id) == $account->id) ? 'selected' : '' }} value="{{ $account->id }}">
                                ...
                                <div class="form-group amount_divs">
                                    ...
                                    <div class="form-control-wrap">
                                        <input type="number" class="form-control cash-collection" id="amount" value="{{ old('amount',$singleCashAmount->amount) }}" name="amount" placeholder="Enter Amount" required>
                                        @error('amount')
                                        <small class="text-danger mt-2">{{ $message }}</small>
                                        @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-control-wrap deliveryman-search">
                                       <textarea class="form-control" name="note" rows="5">{{ $singleCashAmount->note }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
