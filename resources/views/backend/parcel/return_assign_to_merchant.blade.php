<div class="modal fade" id="parcelstatus{{ \App\Enums\ParcelStatus::RETURN_ASSIGN_TO_MERCHANT }}"  role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content" id="data-modal">
            ...
            <form action="{{ route('parcel.return-assign-to-merchant',['page'=>$request->page,'filter'=>$request->filter? $request->filter:'']) }}" method="post">
                @csrf
                <input type="hidden" class="modal_parcel_id" id="modal_parcel_id" value="" name="parcel_id" />
                <div class="modal-body">
                    <div class="form-group  ">
                        <label for="returnassigntomerchant">{{ __('deliveryman.title')}}</label> <span class="text-danger">*</span>
                        <div class="form-control-wrap deliveryman-search">
                            <select id="returnassigntomerchant" class="form-control delivery_man_search" name="delivery_man_id" data-url="{{ route('parcel.deliveryman.search') }}" >
