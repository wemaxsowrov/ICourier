...
<div class="dropdown-menu dropdown-menu-right nav-user-dropdown" aria-labelledby="navbarDropdownMenuLink2">
    ...
    <a class="dropdown-item" href="{{ route('merchant-profile.index', Auth::user()->id) }}"><i
            class="fas fa-user mr-2"></i>{{ __('menus.profile') }}</a>

    <a class="dropdown-item" href="{{ route('merchant.accounts.payment-account.index') }}"><i
            class="fa fa-credit-card mr-2"></i>{{ __('merchant.payment_accounts') }}</a>
    @if (Auth::user()->facebook_id == null && Auth::user()->google_id == null)
        <a class="dropdown-item" href="{{ route('merchant-password.change', Auth::user()->id) }}"><i
